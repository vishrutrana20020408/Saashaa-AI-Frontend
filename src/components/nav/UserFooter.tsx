...
export default function UserFooter() {
  ...
  const userLinks = [
    { name: "Overview", path: "/user" },
    { name: "Dashboard", path: "/user/dashboard" },
    { name: "Interview", path: "/user/interviewdashboard" },
    { name: "Resume", path: "/user/resume" },
    { name: "Need Help", path: "/user/needhelp" },
    { name: "Updates", path: "/user/update" },
  ]

  /* ================= FIXED ACTIVE ROUTE LOGIC ================= */

  const activeIndex = useMemo(() => {
    return userLinks.findIndex((link) => {
      // Overview should only match exact /user
      if (link.path === "/user") {
        return pathname === "/user"
      }

      // Other routes support nested detection
      return (
        pathname === link.path ||
  ...
  return (
    ...
    >
      ...
      <div className="relative border-t border-gray-800 text-center py-4 text-xs sm:text-sm">
        COPYRIGHT © {currentYear} SAASHAA AI. ALL RIGHTS RESERVED.
      </div>
    </motion.footer>
  )
}