...
/**
 * src/app/(public)/user/resume/[resumeId]/versions/[versionId]/edit/page.tsx
 *
 * Backend-integrated Resume Version Edit Page
 *
 ...
 * - aligned with backend-first Resume Management System flow
 * - token + cookie aware requests
 * - safe normalization for mixed payload contracts
 */
...
export default function UserResumeVersionEditPage() {
  ...
  const [tips, setTips] = useState<string[]>([
    "Add measurable achievements and clear outcomes.",
    "Use relevant keywords from the target job description.",
    "Keep section titles ATS-friendly and consistent.",
    "Strengthen projects, experience, and quantified impact.",
  ]);
  ...
  const fetchVersion = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        setErrorMessage(
          error?.message || "Unable to load resume version for editing."
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const saveVersionContent = async () => {
    ...
    try {
      ...
      const updatedVersion = normalizeVersion(unwrapResponse<any>(resultJson));

      if (updatedVersion) {
        hydrateVersion(updatedVersion);
      } else {
      ...
      setSuccessMessage(
        (resultJson as ApiEnvelope<any>)?.message ||
          "Resume version saved successfully."
      );
    } catch (error: any) {
  ...
  const downloadVersion = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to download resume version. Status: ${response.status}`);
      }
  ...
  const resetChanges = () => {
    ...
    setErrorMessage(null);
    setSuccessMessage("Editor reset to last saved version.");
  };
      ...
      <div className="min-h-screen bg-linear-to-br from-gray-950 via-slate-900 to-black px-4 py-10 text-white">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col items-center justify-center gap-4 rounded-3xl border border-white/10 bg-white/5 p-12">
            ...
            <h2 className="text-2xl font-bold">Loading Version Editor</h2>
            <p className="text-white/60">
              Fetching resume version data from your backend.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-white/60">
              Update the selected resume version, save backend-synced content,
              preview it, download it, and recalculate its ATS score.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-12">
          <div className="space-y-6 xl:col-span-8">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl">
              <div className="mb-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-semibold">Version Editor</h2>
                    <p className="text-sm text-white/50">
                      Edit backend-linked raw resume text for this version
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="rounded-xl bg-white/10 px-4 py-2 text-sm font-semibold transition hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    Reset
                  </button>
              ...
              <textarea
                ...
                className="min-h-[520px] w-full rounded-2xl border border-white/10 bg-black/40 p-4 text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-500"
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl">
              ...
              <div className="min-h-55 max-h-[360px] overflow-auto rounded-2xl border border-white/10 bg-black/40 p-4">
                {prettyStructuredJson ? (
                  ...
                  <div className="text-sm text-white/45">
                    No structured content available for this version.
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="space-y-6 xl:col-span-4">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl">
              <div className="mb-4 flex items-center gap-3">
                ...
                <h2 className="text-xl font-semibold">Version Metadata</h2>
              </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl">
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="relative">
                  <svg width="150" height="150" className="h-36 w-36">
                    <circle
                      cx="75"
                      cy="75"
                      r={radius}
                    ...
                    />
                    <defs>
                      <linearGradient id="gradientEdit">
                  ...
                  <div className="absolute inset-0 grid place-items-center">
                    <div className="text-center">
                      ...
                      <div className="mt-1 text-xs text-white/55">ATS</div>
                    </div>
                  </div>
                </div>
                ...
                <button
                  onClick={calculateATS}
                  disabled={scoring}
                  className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-linear-to-r from-green-500 to-blue-600 px-5 py-3 font-semibold transition hover:opacity-90 disabled:opacity-50"
                >
                  {scoring ? (
                    <>
                      <Loader2 size={18} className="animate-spin" />
