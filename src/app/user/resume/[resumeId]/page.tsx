...
/**
 * src/app/(public)/user/resume/[resumeId]/page.tsx
 *
 * Backend-integrated User Resume Detail Page
 *
 ...
 * - compatible with resume/version-oriented architecture
 */
...
const API_ROUTES = {
  ...
  updateResumeContent: (resumeId: string) =>
    `${API_BASE_URL}/api/user/resume/${resumeId}/content`,

  calculateAts: (resumeId: string) =>
    `${API_BASE_URL}/api/user/resume/${resumeId}/ats-score`,

  downloadResume: (resumeId: string) =>
    `${API_BASE_URL}/api/user/resume/${resumeId}/download`,
};
...
type UpdateResumeContentRequest = {
  rawText: string;
};
...
export default function UserResumeDetailPage() {
  ...
  const [tips, setTips] = useState<string[]>([
    ...
    "Highlight projects, skills, and experience clearly.",
  ]);
  ...
  const fetchResume = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        if (!response.ok) {
          // If the specific resume ID lookup fails, fallback to the current latest resume.
          const currentResponse = await fetch(API_ROUTES.getCurrentResume, {
            method: "GET",
            headers: buildAuthHeaders(true),
            credentials: "include",
            cache: "no-store",
          ...
          const currentData = normalizeResumeDetail(
            unwrapResponse<any>(currentJson)
          );
          if (currentResponse.ok && currentData) {
            hydrateResume(currentData);
            return;
          }
        ...
        const resultData = normalizeResumeDetail(unwrapResponse<any>(resultJson));
        ...
        hydrateResume(resultData);
      } catch (error: any) {
        console.error("Fetch resume error:", error);
        setErrorMessage(
          error?.message || "Unable to load resume details from backend."
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const saveResumeContent = async () => {
    ...
    try {
      ...
      const payload: UpdateResumeContentRequest = {
        rawText: resumeText,
      };

      const response = await fetch(API_ROUTES.updateResumeContent(resumeId), {
        method: "PUT",
        headers: buildAuthHeaders(true),
        credentials: "include",
      ...
      if (!response.ok) {
        throw new Error(
          (resultJson as ApiEnvelope<any>)?.message ||
            `Failed to save resume. Status: ${response.status}`
        );
      }
      ...
      setErrorMessage(error?.message || "Failed to save resume content.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const calculateATS = async () => {
    ...
    try {
      ...
      const response = await fetch(API_ROUTES.calculateAts(resumeId), {
        method: "POST",
        headers: buildAuthHeaders(true),
        credentials: "include",
        body: JSON.stringify({
  ...
  const downloadResume = async () => {
    try {
      ...
      const response = await fetch(API_ROUTES.downloadResume(resumeId), {
        method: "GET",
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to download resume. Status: ${response.status}`
        );
      }
      ...
      anchor.download =
        resume?.fileName ||
        `${resume?.resumeName || "resume"}-${resumeId}.pdf`;
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col items-center justify-center gap-4 rounded-3xl border border-white/10 bg-white/5 p-12">
            ...
            <p className="text-white/60">
              Fetching resume details from your backend.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-linear-to-br from-gray-950 via-slate-900 to-black px-4 py-10 text-white">
        <div className="mx-auto max-w-6xl space-y-6">
          ...
          <div className="flex items-start gap-3 rounded-3xl border border-red-500/30 bg-red-500/10 p-6">
            ...
            <p className="text-red-100">Resume not found.</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-white/60">
              View, edit, score, and manage a specific resume inside your
          ...
          <div className="space-y-6 xl:col-span-5">
              ...
              <div className="relative">
                <svg width="180" height="180" className="h-44 w-44">
                  ...
                  <circle
                    ...
                    cy="90"
                    r={radius}
                    stroke="url(#gradientResumeDetail)"
                    strokeWidth="12"
                    fill="transparent"
                  ...
                  />
                  <defs>
                    <linearGradient id="gradientResumeDetail">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <button
                  onClick={() => router.push(`/user/resume/${resumeId}/create-duplicate`)}
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-white/10 px-5 py-3 font-semibold transition hover:bg-white/15"
                >
                  <Copy size={18} />
                  Create Duplicate
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-md">
          <div className="w-full max-w-4xl space-y-4 rounded-2xl border border-white/10 bg-gray-900 p-4 sm:p-6">
            ...
            <p className="text-xs text-white/50">
              This updates the selected resume content in your backend Resume
              Management System.
            </p>
          </div>
        </div>
