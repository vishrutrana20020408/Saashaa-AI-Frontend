...
function getRegisterEndpoint(baseUrl: string): string {
  return `${baseUrl}/api/auth/admin/register`;
}
...
export default function CompanyRegisterPage() {
  ...
  async function handleSubmit(
    ...
    try {
      ...
      const endpoint = getRegisterEndpoint(backendBaseUrl);
      ...
      const role = normalizeRole(
        payload?.userRole ?? payload?.role ?? payload?.roles?.[0] ?? "ADMIN"
      );

      const authId = String(
        payload?.companyId ?? payload?.adminId ?? payload?.userId ?? payload?.id ?? ""
      ).trim();
      ...
      if (token) {
        ...
        window.setTimeout(() => {
          router.replace(getRedirectPath(role || "COMPANY", onboardingDone));
          router.refresh();
        }, 1000);
        return;
      }
    ...
    <div className="min-h-screen bg-(--background) text-(--foreground) flex items-center justify-center p-4">
      ...
      >
        <div className="mb-6 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-500/15 text-indigo-200 shadow-sm">
        ...
        <div className="mt-5 text-center text-sm">
          ...
          <Link href="/auth/login" className="font-medium text-indigo-300 hover:text-indigo-100">
            Sign in
          </Link>
        </div>
